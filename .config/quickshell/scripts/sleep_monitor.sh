...
LOCK_FILE="${XDG_RUNTIME_DIR:-/tmp}/vibeshell-sleep-monitor.lock"
exec 9>"$LOCK_FILE"
flock -n 9 || exit 0
...
get_cmd() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        if [ "$type" == "before" ]; then
            jq -r '.idle.general.before_sleep_cmd // "vibeshell-safe-lock"' "$CONFIG_FILE"
        else
            jq -r '.idle.general.after_sleep_cmd // "vibeshell screen on"' "$CONFIG_FILE"
        fi
    else
        if [ "$type" == "before" ]; then
            echo "vibeshell-safe-lock"
        else
            echo "vibeshell screen on"
        fi
    fi
}
